...
# e.g., make_phylogeny.pl inputfile /data/taxonomy/nodes.dmp /data/taxonomy/names.dmp --verbose
...
open my $infile1, '<', $ARGV[0];		# inputfile
open my $infile2, '<', $ARGV[1];		# nodes.dmp
open my $infile3, '<', $ARGV[2];		# names.dmp
...
while (<$infile1>)
{
	...
	if ( !(defined $hoh{$a[0]}) )
	{
		...
		while ( $i < 32 && $taxid != 1 ) # there's only about 30 possible links max; if taxid == 1, it means we've hit the root and we're at the end
		{
			...
			{
				# print "in loop \n";
				$hoh{$a[0]}{$rank[$taxid]} = $sciname[$taxid];			
			}
