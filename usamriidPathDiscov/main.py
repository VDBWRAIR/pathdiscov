...
pram = [
    [R1, project_dir + "/" + "input" + "/" + pat1 + ".fastq"],
    [R2, project_dir + "/" + "input" + "/" + pat2 + ".fastq"]
]
print yaml.dump(pram)

F_fastq = os.path.abspath(project_dir + "/" + "input" + "/" + pat1 + ".fastq")
R_fastq = os.path.abspath(project_dir + "/" + "input" + "/" + pat2 + ".fastq")
...
def priStage(input, output):
    ...
    result = tasks.stage1(
        F_fastq, R_fastq, project_dir, paramFile, blast_unassembled, output)
    return result
...
def main():
    ...
    pipeline_printout(
        sys.stdout, [
            createPram, prepare_analysis,fastQC,convertToPdf, priStage], forcedtorun_tasks=[
            priStage], verbose=6)
    pipeline_run(
